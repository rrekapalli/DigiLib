...
class _OfflineManagementScreenState extends ConsumerState<OfflineManagementScreen>
      ...
      child: Column(
        ...
        children: [
          ...
          OfflineActionQueueDisplay(
            queueStatus: status.jobQueueStatus,
            pendingJobs: _getMockPendingJobs(), // TODO: Get real jobs from provider
            failedJobs: _getMockFailedJobs(), // TODO: Get real jobs from provider
  ...
  Widget _buildBreakdownItem(String label, String size, double percentage, Color color) {
    final theme = Theme.of(context);
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: _buildDownloadStat('Available Offline', '12', Colors.green),
                ),
                Expanded(
                  child: _buildDownloadStat('Downloading', '2', Colors.orange),
                ),
                Expanded(
                  child: _buildDownloadStat('Failed', '1', theme.colorScheme.error),
                ),
              ],
            ),
          ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ..._getMockDownloadedDocuments().map((doc) => 
              OfflineAvailabilityCard(
                documentId: doc['id'],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Mock action history
            ..._getMockActionHistory().map((action) => 
              ListTile(
                leading: Icon(
                  action['icon'],
                  color: action['color'],
  ...
  void _cleanupCache() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Cache cleanup started...')),
    );
  }
  ...
  void _updateCacheLimit(int newLimitBytes) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Cache limit updated to ${_formatBytes(newLimitBytes)}')),
    );
  }
  ...
  void _cancelJob(String jobId) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Job $jobId cancelled')),
    );
  }
  ...
  void _retryJob(String jobId) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Job $jobId retried')),
    );
  }
  ...
  void _removeDocument(String documentId) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Document $documentId removed from offline storage')),
    );
  }
