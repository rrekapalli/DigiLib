...
class PlatformChannelNativeRenderingWorker implements NativeRenderingWorker {
  static const MethodChannel _channel = MethodChannel('digi_lib_native_rendering');
  ...
  Future<void> _initializePlatformChannel() async {
    try {
      ...
      if (_isAvailable) {
        print('Platform channel native rendering initialized successfully');
      } else {
        print('Platform channel native rendering is not available on this platform');
      }
    } catch (e) {
      _isAvailable = false;
      _isInitialized = true;
      print('Platform channel native rendering initialization failed: $e');
    }
  }
  ...
  Future<Uint8List> renderPage(String filePath, int page, int dpi) async {
    ...
    if (!_isAvailable) {
      throw const NativeRenderingException('Platform channel native rendering is not available');
    }
    ...
    try {
      ...
      if (result == null) {
        throw const NativeRenderingException('Platform channel returned null result');
      }
      
      final success = result['success'] as bool? ?? false;
      if (!success) {
        final errorMessage = result['error'] as String? ?? 'Unknown error';
        throw NativeRenderingException('Platform rendering failed', errorMessage);
      }
      
      final imageData = result['imageData'] as Uint8List?;
      if (imageData == null || imageData.isEmpty) {
        throw const NativeRenderingException('No image data returned from platform');
      }
      ...
      throw NativeRenderingException('Unexpected error during platform rendering', e.toString());
    }
  }
  ...
  Future<String> extractText(String filePath, int page) async {
    ...
    try {
      ...
      if (text == null) {
        throw const NativeRenderingException('No text data returned from platform');
      }
      ...
      throw NativeRenderingException('Unexpected error during text extraction', e.toString());
    }
  }
  ...
  Future<int> getPageCount(String filePath) async {
    ...
    try {
      ...
      if (pageCount == null || pageCount < 0) {
        throw const NativeRenderingException('Invalid page count returned from platform');
      }
      ...
      throw NativeRenderingException('Unexpected error during page count retrieval', e.toString());
    }
  }
  ...
  Future<void> clearCache() async {
    ...
    try {
      ...
      print('Failed to clear platform cache: $e');
    }
  }
