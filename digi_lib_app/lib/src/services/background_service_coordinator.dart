...
class BackgroundServiceCoordinator {
  ...
  void _setupEventListeners() {
    ...
    _taskResultSubscription = _backgroundTaskService.taskResultStream.listen((
      result,
    ) {
  ...
  Future<void> _handleTaskResult(TaskResult result) async {
    debugPrint(
      'Handling task result: ${result.taskId} - ${result.success ? 'success' : 'failed'}',
    );
  ...
  Future<void> _handleJobQueueStatusChange(JobQueueStatus status) async {
    debugPrint(
      'Job queue status changed: ${status.pendingJobs} pending, ${status.failedJobs} failed',
    );
  ...
  Future<void> scheduleDocumentProcessing(
    String documentId,
    ...
    final task = BackgroundTask(
      ...
      payload: {'document_id': documentId, 'options': options},
      scheduledAt: DateTime.now(),
  ...
  Future<void> updateScanProgress(
    String libraryName,
    int progress,
    int total,
  ) async {
  ...
  Future<void> completeScan(String libraryName, int documentsFound) async {
    ...
    await _notificationService.sendScanCompletedNotification(
      libraryName,
      documentsFound,
    );
  }
  ...
  Map<String, dynamic> getStatistics() {
    return {
      ...
      'connectivity': {'is_connected': _connectivityService.isConnected},
    };
  }
