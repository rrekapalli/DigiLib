...
class _FindInDocumentState extends ConsumerState<FindInDocument> {
  ...
  List<SearchMatch> _mockSearch(String query) {
    ...
    for (int page = 1; page <= 5; page++) {
      for (int i = 0; i < (page % 3 + 1); i++) {
        mockMatches.add(SearchMatch(
          pageNumber: page,
          text: query,
          context: 'This is some context text containing $query in the document.',
          startOffset: 40 + i * 10,
          endOffset: 40 + i * 10 + query.length,
          boundingRect: Rect.fromLTWH(50 + i * 20, 100 + i * 25, query.length * 8.0, 20),
        ));
      }
    }
  ...
  void _goToMatch(int index) {
    ...
    ref.read(readerStateProvider(widget.documentId).notifier).goToPage(match.pageNumber);
  }
  ...
  void _previousMatch() {
    if (_matches.isEmpty) return;
    final prevIndex = _currentMatchIndex <= 0 ? _matches.length - 1 : _currentMatchIndex - 1;
...
class SearchHighlightOverlay extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final pageMatches = matches.where((match) => match.pageNumber == currentPageNumber).toList();
    ...
    return Stack(
      children: pageMatches.asMap().entries.map((entry) {
        ...
        final isSelected = selectedMatchIndex != null && 
